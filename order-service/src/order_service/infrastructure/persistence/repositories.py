...
from order_service.domain.exceptions.domain_errors import ConcurrencyError, OrderNotFoundError
from order_service.domain.ports.repositories import PaginatedResult
from order_service.domain.value_objects.money import Money
from order_service.domain.value_objects.order_status import ACTIVE_ORDER_STATUSES, OrderStatus, PaymentStatus
from order_service.infrastructure.persistence.models import OrderItemModel, OrderModel, PaymentModel
...
def _to_domain_order(model: OrderModel) -> Order:
    items = [
        OrderItem(
            ...
            unit_price=Money(Decimal(str(item.unit_price))) if item.unit_price is not None else None,
        )
    ...
    for item in order.items:
        model.items.append(
            OrderItemModel(
                ...
                product_id=item.product_id,
                quantity=item.quantity,
                unit_price=float(item.unit_price.amount) if item.unit_price else None,
            )
        )
...
class SqlAlchemyOrderRepository:
    ...
    async def save(self, order: Order) -> Order:
        if order.version == 1 and not await self.get_by_id(order.id):
            ...
            if model.version != order.version - 1:
                raise ConcurrencyError()
            _apply_order_to_model(order, model)
    ...
    async def find_by_customer(self, customer_id: str, page: int, size: int) -> PaginatedResult:
        offset = (page - 1) * size
        count_result = await self._session.execute(
            select(func.count()).select_from(OrderModel).where(OrderModel.customer_id == customer_id)
        )
    ...
    async def has_active_order(self, customer_id: str, exclude_order_id: UUID | None = None) -> bool:
        query = select(func.count()).select_from(OrderModel).where(
            OrderModel.customer_id == customer_id,
            OrderModel.status.in_([s.value for s in ACTIVE_ORDER_STATUSES]),
        )
        if exclude_order_id:
...
class SqlAlchemyPaymentRepository:
    ...
    async def find_by_order_ids(self, order_ids: list[UUID]) -> dict[UUID, Payment]:
        ...
        return {model.order_id: _to_domain_payment(model) for model in result.scalars().all()}
