...
class Order:
    ...
    def add_item(self, product_id: str, quantity: int) -> OrderItem:
        if not self.can_add_items():
            raise InvalidOrderStateError(
                "Cannot add items to order in current state"
            )
        if quantity <= 0:
            raise ValueError("Quantity must be greater than zero")
        ...
        new_item = OrderItem(
            id=uuid4(), product_id=product_id, quantity=quantity
        )
    ...
    def remove_item(self, item_id: UUID) -> None:
        if not self.can_remove_items():
            raise InvalidOrderStateError(
                "Cannot remove items from order in current state"
            )
    ...
    def confirm_with_prices(self, prices: dict[str, Decimal]) -> None:
        ...
        for item in self.items:
            if item.product_id not in prices:
                raise InvalidOrderStateError(
                    f"Price missing for product {item.product_id}"
                )
            item.unit_price = Money(prices[item.product_id])
        line_items = [(item.product_id, item.quantity) for item in self.items]
        self.total_amount = PricingService.calculate_total(line_items, prices)
    ...
    def apply_payment_approved(self) -> None:
        if self.status != OrderStatus.PAYMENT_PENDING:
            raise InvalidOrderStateError(
                "Order is not awaiting payment result"
            )
    ...
    def apply_payment_rejected(self) -> None:
        ...
        if self.payment_attempts >= MAX_PAYMENT_ATTEMPTS:
            ...
            self.cancellation_reason = (
                "Auto-cancelled after 3 payment rejections"
            )
        else:
...
class Payment:
    ...
    def reinitiate(self, external_reference: str) -> None:
        if self.status != PaymentStatus.REJECTED:
            raise InvalidOrderStateError(
                "Only rejected payments can be reinitiated"
            )
